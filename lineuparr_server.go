...
func (s *lineuparrServer) applyMarketAliases(country, postalCode, preferredSourceID string, inputs []lineuparrbuilder.InputChannel) []lineuparrbuilder.SourceStatus {
	...
	statuses := []lineuparrbuilder.SourceStatus{{
		...
		Message: fmt.Sprintf("%d markets and %d unique lineups scanned; exact station-ID aliases plus confirmed pair-level weekday EPG evidence", snapshot.Summary.CompletedMarkets, snapshot.Summary.Lineups),
	}}
...
func (s *lineuparrServer) handleAliasIndex(w http.ResponseWriter, r *http.Request) {
	...
	if s.store == nil {
		...
		snapshot = s.marketIndex.SnapshotForPostal(config.Gracenote.Country, config.Gracenote.PostalCode)
	}
	response := aliasIndexResponse{Snapshot: snapshot}
	if s.aliasQueue != nil {

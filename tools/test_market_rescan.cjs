...
const scan={marketRank:1,postalCode:'10001',status:'complete',providerAudit:[{family:'spectrum',access:'error'},{family:'spectrum',access:'error'}],sources:[{id:'spectrum-official-lineup',status:'error'}],allProviderYield:{aliases:1,categories:1,currentStations:1},newFamilyYield:{aliases:1,categories:1,currentStations:1}};
...
(async()=>{
 await context.loadMajorMarkets();const button=created.find(n=>n.textContent==='Rescan ZIP 10001');assert.ok(button);assert.equal(button.disabled,false);assert.ok(created.some(n=>String(n.textContent).includes('1 distinct source failures')));assert.ok(created.some(n=>String(n.textContent).includes('new category-evidence facts')));

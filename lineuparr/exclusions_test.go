...
func TestDeniedFullNamesPersistenceScopingAndUndo(t *testing.T) {
	...
	for _, status := range draft.Sources {
		// The composed source-inspection panel counts matched channels; its
		// explanatory message retains the distinct full-name count.
		if status.ID == "dispatcharr-denied" && (status.Matched != 1 || status.Message != fmt.Sprintf("%d denied names at or above 95%% were retained as channel-scoped excluded_aliases", len(want))) {
			t.Fatalf("channel/name counts disagree: %+v", status)
		}
	}

...
`lineuparr_state.json` is not an enrichment cache. It stores explicit inclusion/category choices plus source-scoped TMDB skip/completion and lineup-customization completion. It is ignored automatically when the active Gracenote source fingerprint changes. Channel inclusion, category, duplicate, and restore operations invalidate customization completion.

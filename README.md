...
The optional SD-duplicate action is conservative: it appears when two provider positions map to the same exact sourced identity and one has a stronger HD, UHD, 4K, or digital marker, or when normalized callsigns differ only by a terminal `HD`, `SD`, or unnumbered broadcast `DT` suffix and have one unique strongest variant. A pair such as `WCBS`/`WCBSDT` keeps the `DT` digital/HD position and proposes the otherwise identical base position for removal. Quality-suffix matching requires a base of at least three alphanumeric characters and never strips numbered digital-subchannel suffixes such as `DT2` or `DT3`. Clicking the action opens every proposed remove/keep pair for review; all safe proposals start selected, individual pairs can be unchecked, and only the confirmed subset is excluded. The affected channels remain individually reversible, and **Restore all** puts every provider position back into the export. The `Categorized` and `Needs category` header totals count only channels currently included in the draft.
...
- **Confirm** adds the reviewed stream-name aliases to that specific lineup position with Dispatcharr provenance. Provider-reported `tvg_id` values remain internal matching evidence and are not added by the browser.
...
Provider-reported TVG IDs remain available as internal scoring and provenance evidence, but the browser does not offer TVG-ID checkboxes or add those identifiers to `epg_ids`. Confirming a row still adds its reviewed stream-name aliases. This reflects the current Lineuparr consumer, which matches lineup names and aliases but does not consume lineup JSON `epg_ids`; the server retains the metadata so exact-ID selection can be reconsidered if that behavior changes.

...
Per-lineup/device reports distinguish successful enrichment, no matches, unsupported sources, address requirements, and source/grid failures. Repeated device variants reuse one Spectrum source retrieval during that explicit scan while still matching the result independently against each variant's own Gracenote grid; a later scan retrieves it again. Summary failure totals count distinct source records rather than repeating one failure per device variant. All-provider and new-family-only candidate yields are compared using the same downloaded data, with the selected-lineup comparison reference retained in both. These count new identity/category evidence facts and current station IDs reached, not channels resolved after priority/conflict handling; an unchanged Uncategorized total can therefore be correct. Stored local-scan alias/category totals count only newly retained facts. Repeat-provider skipping remains disabled. Market-specific lineup keys prevent repeated national lineup IDs from overwriting another market's records, and the selected-lineup comparison record is completed, failed, or returned to pending with the scan rather than remaining falsely active. Stop uses the shared scan stop control; interrupted markets are retried on the next click. Keep `MARKET_INDEX_PATH` on persistent storage. Legacy ranked actions remain rejected; the new endpoint runs one curated market only.

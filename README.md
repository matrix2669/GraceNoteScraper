...
priority 4 and qualifying original-language profiles are priority 3. Proposals
remain reviewable unless independent content evidence exceeds the confirmation
threshold and coverage gates; production language alone never confirms broadcast
language. Title-search matches are not exact station evidence.
...
Duplicate review presents whole connected groups of verified SD/HD pairs and exact repeated positions. Checked means Keep. Exactly one position starts checked in each group: the strongest recognized quality, then the lowest numeric channel number, with lexical channel number and stable ID fallbacks. You can check additional positions before applying the review, and at least one included position per group must remain. Exact repeats require the same nonempty Gracenote station ID and callsign; names alone never create these groups.

Channel programme popups close when clicking outside their box. Popups requiring choices, such as export and duplicate review, retain explicit controls. SD suggestions accept multiple HD positions only when those positions share one nonempty Gracenote station ID; the review prefers the strongest quality and then the lowest channel number. Alternate HD or exact-repeat positions start unchecked and will be removed if you apply the review without checking them.
...
| `LINEUPARR_STATE_PATH` | Saved channel choices, match decisions, and source-scoped Lineuparr workflow progress | `lineuparr_state.json` |
...
In both local-ZIP and major-market scans, each provider's official source may use channel numbers only against that provider's own Gracenote grid. Direct accepted categories require matching identity; relationship-backed provisional candidates retain the accepted alias binding and its method. Ambiguous source positions remain protected. Before any number-only aliases are accepted, at least 10 percent of the source's unambiguous positions that overlap the grid—rounded up, with a minimum of one—must have an exact normalized identity at the same channel number. A source that fails this lineup-level check may still contribute safe exact-identity matches, but its number-only aliases are skipped and the source status explains why. Once the check passes, one unique provider-local position may recover descriptive aliases when an official long name differs from an abbreviated Gracenote callsign; its stored alias/category relationship may become a reviewable candidate through the accepted binding, but it does not independently confirm the category. The selected provider is not a special case. Numbers never bridge providers or different market lineups, and EPG comparisons never use an unaffiliated channel-number collision. An exact official identity shared by multiple Gracenote station IDs—including one unambiguous provider row whose same-grid position contains multiple station variants—is kept only as a transient EPG candidate until the existing two-block schedule confirmation succeeds. Source adapters must resolve the appropriate regional/address-qualified source where required. Older number-only facts without the source-alignment marker remain quarantined. Run a fresh scan to obtain corrected evidence; saved exports change only when explicitly exported again.
...
- A cross-ID pair requires a shared normalized callsign, non-generic affiliate/network, or attributable official provider name. One unambiguous official provider row may supply that exact name to multiple station variants at its same-grid position for comparison only; the name is not persisted by itself. Schedule-only and unaffiliated number collisions never create aliases. Both selected six-hour blocks must have at least 80 percent meaningful coverage and at least 80 percent matching coverage. Confirmation requires six matched programme occurrences across the blocks, or the long-form exception of two titles and ten matched hours.
...
- Official provider aliases and categories retain their source URL and exact join method. Categories and ambiguous aliases shared by multiple station IDs are not applied automatically; exact shared identities are transient candidate evidence and become cross-ID aliases only after weekday EPG confirmation.
...
| Optimum | NY/NJ/CT/PA/selected-NC market PDFs or the public address-qualified Suddenlink/Optimum services | Eastern PDFs contribute explicit section categories via verified identity and corroborated numbers within their own provider grid; western service areas require a user-selected address for an exact local lineup |
...
Category review also supports row checkboxes, **Select all shown**, and **Confirm selected**. Filter the pending rows by proposed category, then sort each priority group by channel number, channel name, or category. **Select all shown** applies only to the current filtered result so hidden rows are not confirmed accidentally. Changing a row's category automatically selects it without saving immediately. **Commit** saves one row, while **Confirm selected** atomically saves every selected row with its currently displayed category, including different corrections across the batch. Failed batches retain the pending edits and selection for retry. Excluded or already-reviewed channels are not eligible, and stale selections are rejected without partially saving the batch.

Use the separate collapsible **Category review** section for included channels with provisional assignments. Pending rows are grouped by evidence priority, including stronger-priority proposals with unresolved provider disagreements. Each row leads with channel number and callsign, adds the best available descriptive name, links to the selected guide's upcoming-programme popup when the LineupARR builder feature is composed, and displays its proposed category and expandable source evidence. Leave the proposed category unchanged or choose a correction, then use the row's **Commit** button or the batch **Confirm selected** action. Successful saves remove rows and reduce the remaining count; manual choices and original-proposal review history persist. The Channels section retains ordinary channel editing without inline confirmation buttons.

TMDB category evidence is a separate collapsible section before **Category review**. **Scan TMDB enrichment data** reuses retained programme genre IDs, original-language values, or older cached genre names when the programme already has the same TMDB movie/series identity. This is a local metadata scan, not a guide rebuild or a TMDB download. You do not need to clear the cache or reset your lineup; manual category choices remain unchanged. For older provider scans that did not retain Gracenote's response-level timezone, the first category scan performs one lightweight provider-discovery lookup and repairs the saved timezone without downloading guide grids. Programme matches without usable evidence await normal enrichment. Genre proposals remain priority 4. A predominantly non-English profile may propose priority-3 `International` only with more than 60% non-English covered weekday airtime, at least 50% coverage, eight covered weekdays and eight distinct matched titles. TMDB proposals remain reviewable unless independent content evidence meets the strict >80% confirmation rule and its coverage gates. Production-language evidence alone never confirms broadcast language.
...
Provider scans retain alias/category relationships with source provenance, including aligned provider-local alias recovery. A complete successful source refresh replaces that scope's relationships; failed, cancelled, skipped or unattempted retrieval keeps its last-known-good evidence. Existing data does not need to be deleted. After upgrading, explicitly rescan older local/major-market results to collect categories that previous versions discarded. Original programme-category provenance becomes available when the guide is normally rebuilt; mixed legacy guide labels are not treated as independent confirmation. Saved schedule confirmations are tied to the complete programme evidence and the verified lineup timezone; changed evidence, a conflicting current timezone, or legacy results without timezone provenance require a fresh explicit category scan. These changes do not refresh streams or alter the Dispatcharr 70% matching / 95% alias-exclusion policy.

...
| `SPECTRUM_CATALOG_PATH` | Persistent last-known-good Spectrum national TMS-ID catalog | `spectrum_catalog_cache.json` beside market index |
| `SPECTRUM_CATALOG_REFRESH_TOKEN` | Change to a new non-empty value for one early Spectrum refresh attempt | disabled |
...
The experimental **Major-market enrichment** section scans exactly one market per click, starting at the first unfinished market in the retained 2025-26 ranked ZIP list. No automatic or 25-market runs occur. Each market uses the local provider-source/weekday EPG pipeline and a transient grid of the selected lineup for comparison; it never changes the selected provider or publishes another full guide. Address-required sources are skipped unless that market has a deliberately validated public reference address for the same provider family; the subscriber's local address is never forwarded. Chicago uses ZIP 60611 and the public Trump International Hotel & Tower address for Xfinity only. Reference addresses remain in-memory scan inputs and never enter the index, snapshots, logs, caches, exports, or API views. Existing AFN/Glorystar exclusions remain.

Per-lineup/device reports distinguish successful enrichment, no matches, unsupported sources, address requirements, and source/grid failures. Spectrum's national catalog is reused across every successfully fetched provider grid by exact Gracenote station ID, including grids whose provider-local source is unavailable; it does not use or assert local Spectrum channel numbers. Its station-bound aliases/categories never become cross-station EPG evidence. The bundled release snapshot and persistent last-known-good cache prevent repeated retrievals during incremental scans. Summary failure totals count distinct source records rather than repeating one failure per device variant. All-provider and new-family-only candidate yields are compared using the same downloaded data, with the selected-lineup comparison reference retained in both. These count new identity/category evidence facts and current station IDs reached, not channels resolved after priority/conflict handling; an unchanged Uncategorized total can therefore be correct. Stored local-scan alias/category totals count only newly retained facts. Repeat-provider skipping remains disabled. Market-specific lineup keys prevent repeated national lineup IDs from overwriting another market's records, and the selected-lineup comparison record is completed, failed, or returned to pending with the scan rather than remaining falsely active. Stop uses the shared scan stop control; interrupted markets are retried on the next click. Keep `MARKET_INDEX_PATH` and `SPECTRUM_CATALOG_PATH` on persistent storage. Legacy ranked actions remain rejected; the new endpoint runs one curated market only.
...
- Runtime adapters cover Verizon FiOS, Optimum, DIRECTV, DISH, AT&T U-verse, Xfinity, Spectrum, and BroadStar. Provider-local adapters parse the provider's public source during an explicit scan. Spectrum instead ships a reviewed national snapshot and checks its official TMS-ID catalog no more than once every seven days, retaining the last-known-good data when the live request is unavailable. A durable refresh reservation is written before requesting it; if the reservation cannot be saved, the current snapshot is served without a request. A complete validated catalog revision may replace or retire only prior Spectrum-owned facts, never manual or other-source evidence. Other reviewed compatibility snapshots remain disabled unless `LINEUPARR_REFERENCE_CATALOGS=on` is explicitly set. AFN and Glorystar are excluded from enrichment scans and source listings, but remain selectable as the main guide provider. Existing index files are not deleted; their direct provider facts and provider-only names are ignored.
...
| Spectrum | Official national channel catalog | Ships 378 reviewed channel names/categories from the public page and joins every fetched grid only by exact Spectrum TMSID/Gracenote station ID. The catalog supplies no local availability, channel-number, or cross-station EPG evidence. Live checks are limited to once per seven days; failures retain the last-known-good or bundled snapshot. |
...
Provider-source failures do not interrupt guide generation, invalidate successfully downloaded Gracenote lineups, or prevent a Gracenote-only export. Optional Lineuparr catalog downloads have their own 24-hour cache; official provider adapters run during an on-demand ZIP scan or an explicit Save & test address lookup. Spectrum uses `SPECTRUM_CATALOG_PATH` beside the market index by default. Change `SPECTRUM_CATALOG_REFRESH_TOKEN` to a new non-empty value for one deliberate early refresh attempt; retaining the same value across restarts does not repeat it. The cache path must be persistent and writable: if its refresh reservation cannot be saved, no live Spectrum request is attempted and the current bundle/cache remains active. Source URLs are server configuration; credentials and stream URLs are never part of the exported JSON.
